...
const Board = ({
      ...
      setTimeout(() => {
        ...
        for (let [index, code] of assertion[row].entries()) {
          clone.set(code, {
            ...clone.get(code),
            buttons:
              inputs[row][index] + " " + clone.get(code).buttons.trim().slice(),
          });
        }
  ...
  const editCell = useCallback((row, key) => {
    ...
    if (!ALPHABET[key] && inputs[row][editableCellRef.current[1]] !== "-")
      placeHolderCounter++;
              ...
              {row_el.map((col_el, col_index) => {
                ...
                return (
                  <div
                    ...
                    className={`cell ${assertion[row_index][col_index]} ${
                      editableCell === cell_id ? "glow" : ""
                    }`}
                  >
                    {col_el}
                  </div>
